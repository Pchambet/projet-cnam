...
\subsection{Distance pond\'er\'ee ($D_\omega$)}

On peut combiner les deux en une distance pond\'er\'ee~:
\[
    D_\omega(\hat{X}_i, \hat{X}_j) = \sqrt{\omega \cdot D_0^2 + (1 - \omega) \cdot D_1^2}, \qquad \omega \in [0, 1].
\]
\begin{itemize}[nosep]
    \item $\omega = 1$~: seul le \textbf{niveau} des courbes compte,
    \item $\omega = 0$~: seule la \textbf{forme} (d\'eriv\'ee) compte,
    \item $\omega \in (0,1)$~: compromis entre niveau et forme.
\end{itemize}

Ces distances permettent de construire une matrice de distances $n \times n$, utilisable directement par la CAH ou le k-medoids.
...
\[
    \text{Individu } i~: \quad \underbrace{\hat{X}_i(t)}{\text{courbe (fonctionnel)}} \quad + \quad \underbrace{Z_i = (z_{i1}, \ldots, z_{ip})}{\text{vecteur (classique)}}.
\]
...
\subsection{Approche par distance pond\'er\'ee $D_\omega$}

On calcule \textbf{s\'epar\'ement} une distance fonctionnelle et une distance vectorielle, puis on les combine~:
\begin{align*}
    D_f(i,j) &= \sqrt{\int_{\mathcal{T}} \bigl[\hat{X}_i(t) - \hat{X}_j(t)\bigr]^2 \, dt} \qquad \text{(distance $L^2$ entre courbes)}, \\
    D_s(i,j) &= \| Z_i - Z_j \| \qquad \text{(distance euclidienne entre vecteurs)}, \\
    D_\omega(i,j) &= \sqrt{\omega \cdot D_f(i,j)^2 + (1 - \omega) \cdot D_s(i,j)^2}, \qquad \omega \in [0, 1].
\end{align*}

Le param\`etre $\omega$ contr\^ole l'\textbf{importance relative} des courbes par rapport aux variables classiques~:
\begin{itemize}[nosep]
    \item $\omega = 1$~: clustering uniquement sur les courbes,
    \item $\omega = 0$~: clustering uniquement sur les variables vectorielles,
    \item $\omega \in (0,1)$~: combinaison des deux sources.
\end{itemize}
La question du \textbf{choix optimal de $\omega$} est l'un des sujets de recherche du stage.
...
\paragraph{Avantage.} Pas de param\`etre $\omega$ \`a choisir~; la combinaison est plus \guillemotleft~naturelle~\guillemotright.

\paragraph{Inconv\'enient.} Il faut choisir les fonctions noyaux $K_f$ et $K_s$ ainsi que leurs param\`etres (ex.~: la largeur de bande $\sigma$ du noyau gaussien).
...
\begin{tabular}{llll}
    ...
    Entr\'ee & Vecteurs $\xi_i \in \R^K$ & Matrice de distances $D_\omega$ & Matrice de distances $D_K$ \\
    Donn\'ees mixtes & Concat\'enation $(\xi_i, Z_i)$ & $D_\omega$ pond\'er\'ee & Produit $K_f \cdot K_s$ \\
    Param\`etres & $K$ (nb de scores) & $\omega$ (poids) & Noyaux et $\sigma$ \\
    \hline
\end{tabular}
\end{center}

\medskip
Le travail du stage consiste \`a \textbf{comparer ces trois approches} --- d'abord sur des donn\'ees simul\'ees (o\`u les vrais clusters sont connus), puis sur les donn\'ees oc\'eanographiques r\'eelles --- afin de d\'eterminer quelle strat\'egie produit les clusters les plus pertinents du point de vue de la propagation acoustique.
